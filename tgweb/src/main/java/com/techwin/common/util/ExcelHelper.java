...
/**
 * 
 * @author jk getted from http://jxls.cvs.sourceforge.net/jxls/jxls/src/java/org/jxls/util/Util.java?revision=1.8&view=markup by Leonid Vysochyn and modified
 *         (adding styles copying) modified by Philipp Löpmeier (replacing deprecated classes and methods, using generic types)
 */
public final class ExcelHelper {
	...
	public static void copySheets(HSSFSheet newSheet, HSSFSheet sheet, boolean copyStyle) {
		...
		for (int i = sheet.getFirstRowNum(); i <= sheet.getLastRowNum(); i++) {
			...
			if (srcRow != null) {
				ExcelHelper.copyRow(sheet, newSheet, srcRow, destRow, styleMap);
	...
	/**
	 * Récupère les informations de fusion des cellules dans la sheet source pour les appliquer à la sheet destination... Récupère toutes les zones merged dans
	 * la sheet source et regarde pour chacune d'elle si elle se trouve dans la current row que nous traitons. Si oui, retourne l'objet CellRangeAddress.
	 * 
	 * @param sheet
