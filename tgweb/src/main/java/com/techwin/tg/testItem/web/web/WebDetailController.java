...
public class WebDetailController {
	...
	public String getWebDetailTab(Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		return "/tg/testItem/web/WebDetail";
	}

	@RequestMapping(params = RequestConstants.SEARCH)
	public @ResponseBody JqGridMapper getWebDetailList(HttpServletRequest request, HttpServletResponse response) throws Exception {
		
		logger.debug("\nWeb detail 리스트 검색\n");
		
		return RequestUtil.generateJqGridResponse(request, webDetailService);
	}

	@SuppressWarnings("unchecked")
	@RequestMapping(params = RequestConstants.EXCEL)
	public void getWebDetailExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
		
		logger.debug("\nWeb detail export excel\n");
		
		List<Map<String, Object>> list = webDetailService.getWebDetailExcel(RequestUtil.bindParameterToMap(request.getParameterMap()));
