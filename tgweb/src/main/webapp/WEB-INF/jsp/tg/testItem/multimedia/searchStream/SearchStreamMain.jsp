...
$(function() {
	$.Topic("dataGrid/xhrComplete/" + "tgSearchStreamMain.do").subscribe(function sub() {
...
*/
		...
		$.Topic("dataGrid/xhrComplete/" + "tgSearchStreamMain.do").unsubscribe(sub);
	});
});
</script>
<form id="${_wid}_searchForm">
<c:if test="${empty params}">
	<input type="hidden" name="TEST_PROJECT_ID" value="<spring:message code="tgweb.common.SEARCH_STREAM"/>" />
  	<div>
		<table class="lTable">
			<tr>
				<th>Test Project</th>
				<td><spring:message code="tgweb.common.SEARCH_STREAM"/></td>
			</tr>
		</table>
	</div>
</c:if>
...
<c:if test="${not empty params}">
	...
	<div>
		<table id="${_wid}_grid" class="grid" data="{wid:'${_wid}', formId:'${_wid}_searchForm', url:'tgSearchStreamMain.do', postData:{method:'s'}, key:'TC_IDX', caption:'<spring:message code="tgweb.common.SEARCH_STREAM.title"/> Main', pager:'#${_wid}_pager'}">
			<tr>
<c:if test="${empty params}">
				...
				<th field="TEST_CASE_COUNT" url="tgSearchStreamTc.do" addParam="{'method':'t', 'tabTitle':'TEST_SUITE_ID'}" uid="SearchStreamMain" width="80px;">Total TC</th>
<!-- 
 ...
 -->
				...
				<th field="TEST_CASE_FAIL_COUNT" syntax="{'matcher':'NOT_MATCH', 'flag':0, 'syntaxColor':'red'}" url="tgSearchStreamTc.do" addParam="{'method':'t', 'tabTitle':'TEST_SUITE_ID', 'filterColumn':'RESULT', 'filterValue':'FAIL'}" width="60px">FAIL</th>
