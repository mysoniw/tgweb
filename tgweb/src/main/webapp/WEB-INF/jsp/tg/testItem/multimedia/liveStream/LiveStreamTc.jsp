...
$(function() {
	$.Topic("dataGrid/xhrComplete/" + "tgLiveStreamTc.do").subscribe(function sub(data) {
		...
		$.Topic("dataGrid/xhrComplete/" + "tgLiveStreamTc.do").unsubscribe(sub);
	});
});
</script>
<form id="${_wid}_searchForm">
	...
	<input type="hidden" name="IS_WEB" value="${params.IS_WEB}" />
  	<div>
		<table class="lTable">
			<tr>
				<th>Category</th>
...
<c:if test="${params.CATEGORY == 'DVR'}">                                     
	...
	<div>
		<table id="${_wid}_grid" class="grid" data="{wid:'${_wid}', formId:'${_wid}_searchForm', url:'tgLiveStreamTc.do', postData:{method:'s'}, key:'TC_IDX', caption:'<spring:message code="tgweb.common.LIVE_STREAM.title"/> TestCase', pager:'#${_wid}_pager'}"
...
<c:if test="${params.CATEGORY == 'DVR'}">
				...
				<th field="TOTAL_CYCLE" url="tgLiveStreamCycle.do" addParam="{'method':'t', 'tabTitle':'TEST_CASE_ID'}" uid="LiveStreamTc" width="80px;">Total Cycle</th>
<c:if test="${params.CATEGORY == 'NCAM'}">
	<c:if test="${params.EXTRA3 == 'H.264' || params.EXTRA3 == 'MPEG4'}">
				...
				<th field="FAIL" syntax="{'matcher':'NOT_MATCH', 'flag':0, 'syntaxColor':'red'}" url="tgLiveStreamCycle.do" addParam="{'method':'t', 'tabTitle':'TEST_CASE_ID', 'filterColumn':'RESULT', 'filterValue':'FAIL'}" width="60px">FAIL</th>
