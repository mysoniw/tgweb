...
<c:if test="${params.CATEGORY == 'NVR'}">                                     
			<li><select id="${_wid}_extra3" name="EXTRA3" class="select w110px" data="{formId:'${_wid}_searchForm', url:'cmnCommon.do?method=groupSelect', emptyLabel:'Codec', data:{target:'EXTRA3'}}"></select></li>
			<li><select id="${_wid}_extra5" name="EXTRA5" class="select w120px" data="{formId:'${_wid}_searchForm', url:'cmnCommon.do?method=groupSelect', emptyLabel:'Framerate', data:{target:'EXTRA5'}}"></select></li>
</c:if>                                                                       
<c:if test="${params.CATEGORY == 'DVR'}">                                     
	...
	<div>
		<table id="${_wid}_grid" class="grid" data="{wid:'${_wid}', formId:'${_wid}_searchForm', url:'tgImageConfirmTc.do', postData:{method:'s'}, key:'TC_IDX', caption:'<spring:message code="tgweb.common.IMAGE_CONFIRM.title"/> TestCase', pager:'#${_wid}_pager'}"
...
<c:if test="${params.CATEGORY == 'DVR'}">
				...
				<th field="TOTAL_CYCLE" url="tgImageConfirmCycle.do" addParam="{'method':'t', 'tabTitle':'TEST_CASE_ID'}" uid="ImageConfirmTc" width="80px;">Total Cycle</th>
				<th field="RESULT" syntax="{'matcher':'NOT_MATCH', 'flag':'PASS','syntaxColor':'red'}" width="80px">Result</th>
				<th field="PASS" width="60px">PASS</th>
				<th field="FAIL" syntax="{'matcher':'NOT_MATCH', 'flag':0, 'syntaxColor':'red'}" url="tgImageConfirmCycle.do" addParam="{'method':'t', 'tabTitle':'TEST_CASE_ID', 'filterColumn':'RESULT', 'filterValue':'FAIL'}" width="60px">FAIL</th>
