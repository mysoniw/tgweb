...
<form id="${_wid}_searchForm">
	...
	<div class="searchDiv">
		<ul>
			<li><select id="${_wid}_extra1" name="EXTRA1" class="select w180px" data="{formId:'${_wid}_searchForm', url:'cmnCommon.do?method=groupSelect', emptyLabel:'Protocol', data:{target:'EXTRA1'}}"></select></li>
	...
	<div>
		<table id="${_wid}_grid" class="grid" data="{wid:'${_wid}', formId:'${_wid}_searchForm', url:'tgRs485Tc.do', postData:{method:'s'}, key:'TC_IDX', caption:'<spring:message code="tgweb.common.RS_485.title"/> TestCase', pager:'#${_wid}_pager'}"
			filterGrid="{'column':'${params.filterColumn}', 'value':'${params.filterValue}', 'len':'${params.TEST_CASE_FAIL_COUNT}'}">
			<tr>
				<th field="TEST_CASE_NAME" width="400px">TC</th>
				<th field="EXTRA1" width="120px">Protocol</th>
				<th field="TOTAL_CYCLE" url="tgRs485Cycle.do" addParam="{'method':'t', 'tabTitle':'TEST_CASE_ID'}" uid="Rs485Tc" width="80px;">Total Cycle</th>
				<th field="RESULT" syntax="{'matcher':'NOT_MATCH', 'flag':'PASS','syntaxColor':'red'}" width="80px">Result</th>
				<th field="PASS" width="60px">PASS</th>
				<th field="FAIL" syntax="{'matcher':'NOT_MATCH', 'flag':0, 'syntaxColor':'red'}" url="tgRs485Cycle.do" addParam="{'method':'t', 'tabTitle':'TEST_CASE_ID', 'filterColumn':'RESULT', 'filterValue':'FAIL'}" width="60px">FAIL</th>
