...
$(function() {
	...
	$.Topic("dataGrid/xhrComplete/" + "tgLiveStreamDetail.do").subscribe(function xhrSub(_data) {
		data = _data;
		
		fnInit();
		
		$.Topic("dataGrid/xhrComplete/" + "tgLiveStreamDetail.do").unsubscribe(xhrSub);
	});
	...
	chart = {
		...
		title: {
			text: "${params.MODEL} (${params.FW_VERSION}ver) - Live Stream"
		},
		subtitle: {
...
<form id="${_wid}_searchForm">
	...
	<div class="accordion">
		...
		<c:if test="${params.CATEGORY == 'NCAM' && params.EXTRA3 != 'MJPEG'}">
						<th>GOP Count</th>
		</c:if>
					</tr>
					<tr>
		...
		<c:if test="${params.CATEGORY == 'NCAM' && params.EXTRA3 != 'MJPEG'}">
						<td <c:if test="${params.GOP_COUNT == 'FAIL'}">style="color:#FF0000;"</c:if>>${params.GOP_COUNT} (Lowest Value: ${params.EXTRA18}, Avg Value: ${params.EXTRA19})</td>
		</c:if>
					</tr>
				</table>
			</div>
		...
		<div class="accordionPart closed">
			...
			<div>
				...
				<div style="float:left;">
					<table id="${_wid}_grid" class="grid" data="{wid:'${_wid}', recordtext:'', loadonce:true, rownumbers:false, formId:'${_wid}_searchForm', url:'tgLiveStreamDetail.do', postData:{method:'s'}, key:'TC_IDX', caption:'<spring:message code="tgweb.common.LIVE_STREAM.title"/> Detail', pager:'#${_wid}_pager'}">
						<tr>
						<c:if test="${params.CATEGORY != 'DVR'}">
							<th field="SEQ" width="40px">Time</th>
							<th field="F_COUNT" syntax="{'matcher':'RANGE', 'flag':['${params.EXTRA5}', '${params.EXTRA13}'],'syntaxColor':'red'}" width="100px">Frame Count</th>
							<th field="F_DEVIATION" syntax="{'matcher':'OVER', 'flag':'${params.F_DEV_TOLERANCE_BASE}','syntaxColor':'red'}" width="100px">Frame Deviation</th>
							<th field="PC_DEVIATION" syntax="{'matcher':'OVER', 'flag':'${params.PC_DEV_TOLERANCE_BASE}','syntaxColor':'red'}" width="100px">PC Deviation</th>
							<th field="F_REVERSE" syntax="{'matcher':'NOT_MATCH', 'flag':'0','syntaxColor':'red'}" width="100px">Frame Reverse Count</th>
						</c:if>
						<c:if test="${params.CATEGORY == 'DVR'}">
							<th field="SEQ" width="40px">Time</th>
							<th field="F_COUNT" width="100px">Frame Count</th>
							<th field="F_DEVIATION" width="100px">Frame Deviation</th>
							<th field="PC_DEVIATION" width="100px">PC Deviation</th>
							<th field="F_REVERSE" width="100px">Frame Reverse Count</th>
						</c:if>
						</tr>
					</table>
