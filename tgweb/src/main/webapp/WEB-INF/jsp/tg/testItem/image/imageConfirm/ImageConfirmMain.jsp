...
<c:if test="${empty params}">
	<input type="hidden" name="TEST_PROJECT_ID" value="<spring:message code="tgweb.common.IMAGE_CONFIRM"/>" />
  	<div>
		<table class="lTable">
			<tr>
				<th>Test Project</th>
				<td><spring:message code="tgweb.common.IMAGE_CONFIRM"/></td>
			</tr>
		</table>
	</div>
</c:if>
...
<c:if test="${not empty params}">
	...
	<div>
		<table id="${_wid}_grid" class="grid" data="{wid:'${_wid}', formId:'${_wid}_searchForm', url:'tgImageConfirmMain.do', postData:{method:'s'}, key:'TC_IDX', caption:'<spring:message code="tgweb.common.IMAGE_CONFIRM.title"/> Main', pager:'#${_wid}_pager'}">
			<tr>
<c:if test="${empty params}">
				...
				<th field="TEST_CASE_COUNT" url="tgImageConfirmTc.do" addParam="{'method':'t', 'tabTitle':'TEST_SUITE_ID'}" uid="ImageConfirmMain" width="80px;">Total TC</th>
				<th field="TEST_CASE_PASS_COUNT" width="60px">PASS</th>
				<th field="TEST_CASE_FAIL_COUNT" syntax="{'matcher':'NOT_MATCH', 'flag':0, 'syntaxColor':'red'}" url="tgImageConfirmTc.do" addParam="{'method':'t', 'tabTitle':'TEST_SUITE_ID', 'filterColumn':'RESULT', 'filterValue':'FAIL'}" width="60px">FAIL</th>
