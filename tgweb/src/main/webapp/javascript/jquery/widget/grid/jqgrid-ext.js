...
$.jgrid.ext = {
	...
	ajaxFormProxy : function(opts, act) {
		//console.debug("ajaxFormProxy invoke");
		// get url
		opts.url = $(this).getGridParam('url') + "?method=up";
...
$.extend($.jgrid.edit, {
	...
	recreateForm : true
// recreating form removes a lot of butthurt of form editing
});
...
$.jgrid.extend({
	...
	'extFooterAgg' : function() {
		return this.each(function() {
			var $grid = $(this);
			if (typeof $grid.jqGrid('getGridParam', 'userData')['agg'] != 'object')
				return;
	...
	'extExport' : function(data, success) {
		return this.each(function(type) {
			...
			var $frame = $('<iframe src="' + url + '&' + $.param($.extend(null, postData, {
				'_out' : 'export'
			}, data)) + '" style="display:none;"></iframe>');
	...
	'extGroupHeader' : function(opts) {
		...
		$t.p.resizeStop = function(nw, idx) {
			...
			if ($.isFunction(resizeStop))
				resizeStop.call(this, nw, idx);
		};
	},
	...
	'updateGroupHeader' : function() {
		return this.each(function() {
			...
			var $first_row = $labels.clone().removeClass('ui-jqgrid-labels').removeAttr('role').addClass('ui-jqgrid-labels-firstrow');
			...
			for ( var i in $t.p.colModel) {
				...
				if (col.hidden)
					continue;
				if (prev_hgroup === null)
					prev_hgroup = hgroup; // first non-hidden column becomes
											// initial group
				...
				if (prev_hgroup == hgroup) {
					...
					if ($t.p.groupHeader[prev_hgroup])
						$th.text($t.p.groupHeader[prev_hgroup].label);
